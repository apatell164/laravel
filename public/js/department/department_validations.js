var Validate = function () {  
    var handleCoupons = function () {
        ...
        $('#frmDept input').on('keypress', function (e) {
            if (e.keyCode == 13 && e.keyCode != 188) {
                if ($('#frmDept').validate().form()) {
                    $('#frmDept').submit();
                }
                return false;
            }
...
jQuery.validator.addMethod("description", function (value) {
    var userAgent = navigator.userAgent.toLowerCase();
    regEx = /^[a-zA-Z 0-9 , \'\â€™\â€˜\`\-\â€“\â€”\_\.\%\ ]*$/;
...
jQuery.validator.addMethod("validateContentLength", function (value, element) {
    var charLimit = 1000; 
    var strippedContent = value.replace(/<[^>]*>/g, ''); 
...
},"You can't add more then 1000 Characters");



jQuery.validator.addMethod("pricerange", function (value, element) {

    var radioValue = $("input[name='price_type']:checked").val();
    var orignal = $("#varMinimumOrderAmount").val();
    var discount = $("#commission_value").val(); 
    if(discount > 0){
        if (radioValue == 'P') {
            if (isNaN(discount) || discount < 0 || discount > 100) {
                $.validator.messages.pricerange = "Percentage discount must be between 0 and 100.";
                return false
            }else{
                return true;
            }
        }else if (radioValue == 'F'){
            $.validator.messages.pricerange = "Flat amount cannot be greater than the orignal price.";
            return parseFloat(orignal) >= parseFloat(discount) && discount > 0
        }
    }else{
        if (radioValue == 'P') {
            $.validator.messages.pricerange = "Percentage discount value must be greater than 0.";
        }
       
        if (radioValue == 'F'){
            $.validator.messages.pricerange = "Flat amount value must be greater than 0.";
        }
        return false;
    }
...
$(document).on('keyup', '#varMinimumOrderAmount', function (event) {
    var input = event.currentTarget.value;
    if (input.search(/^0/) != -1) {
        $("#varMinimumOrderAmount").val('');
    }
});

$('#varMinimumOrderAmount').keypress(function (eve) {
    if ((eve.which != 46 || $(this).val().indexOf('.') != -1) && (eve.which < 48 || eve.which > 57) || (eve.which == 46 && $(this).caret().start == 0)) {
        eve.preventDefault();
    }
    $('#varMinimumOrderAmount').keyup(function (eve) {
...
}, 'Value must be a number greater than zero (0)');
...
jQuery.validator.addMethod("allowed_special_character_name", function (value) {
    var userAgent = navigator.userAgent.toLowerCase();
    regEx = /^[a-zA-Z 0-9\'\â€™\â€˜\`\-\â€“\â€”\_\%\ ]*$/;
    if (!regEx.test(value) && value != '') {
        return false;
